...
import bridgePassioRecognitionResultStatus
import bridgePassioResultPassioAdvisorFoodInfos
import bridgePassioResultPassioGeneratedMealPlan
import bridgePassioSpeechRecognitionModel
import bridgePassioUPFRatingResponse
...
class PassioSDKBridge(reactContext: ReactApplicationContext) :
  ...
  fun configure(
    ...
    mainHandler.post {
      val config = PassioConfiguration(reactApplicationContext, key).apply {
        ...
        this.allowInternetConnection = allowInternetConnection ?: true
        this.proxyUrl = proxyUrl
        this.proxyHeaders = proxyHeaders.toMapString()
  ...
  fun generateMealPlan(
    ...
    PassioSDK.instance.generateMealPlan(
      request = request,
    ) {result->
      promise.resolve(bridgePassioResultPassioGeneratedMealPlan(result))
    }
  }

  @ReactMethod
  fun generateMealPlanPreview(
    request: String,
    promise: Promise,
  ) {
    PassioSDK.instance.generateMealPlanPreview(
      request = request,
    ) {result->
      promise.resolve(bridgePassioResultPassioGeneratedMealPlan(result))
    }
  ...
  @ReactMethod
  fun recognizeSpeechRemoteWithGrouping(
    request: String,
    promise: Promise,
  ) {
    PassioSDK.instance.recognizeSpeechRemoteWithGrouping(
      text = request,
    ) {result->
      promise.resolve(bridgePassioRecognitionResultStatus(result))
    }
  }

  @ReactMethod
  fun recognizeImageRemoteWithGrouping(
    imageUri: String,
    message: String?,
    resolution: String,
    promise: Promise,
    ...
    if (imageBitmap == null) {
      ...
      PassioSDK.instance.recognizeImageRemoteWithGrouping(
        imageBitmap,
        message = message,
        resolution = PassioImageResolution.valueOf(resolution)
      ) { result ->
  ...
  fun fetchUltraProcessingFoodRating(
    ...
    val passioFoodItem = bridgePassioFoodItemMapper(response)
    if(passioFoodItem!== null){
      PassioSDK.instance.fetchUltraProcessingFoodRating(passioFoodItem) { result ->
        promise.resolve(bridgePassioUPFRatingResponse(result))
      }
    }else{
      promise.reject(Throwable("unable to convert"))
